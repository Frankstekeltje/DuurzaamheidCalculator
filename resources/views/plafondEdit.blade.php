...
@section('body')
        ...
        @for ($i = 0; $i < $saveAmount; $i++)
                    ...
                    @if ($prevMaterialType == "")
                        ...
                        <optgroup label="{{$material->type}}">
                            @if($material->type != "Luchtspouw")
                                ...
                                @endif
                            @else
                                @if(strpos($material->name, 'plafond') !== false)
                                    @if($id == $material->id)<option selected value="{{$material->id}}">{{$material->name}}</option>
                                    @else <option value="{{$material->id}}">{{$material->name}}</option>
                                    @endif
                                @endif
            ...
            <br>
        @endfor
        <input value="{{$gebouw->name}}" placeholder="Naam van plafond..." type="text" name="naam">
        <br>
        <input type="hidden" value="plafond" name="type">
        <br>
        <button type="submit" name="submitButton" class="btn btn-outline-success">Pas plafond aan</button>
    </form>
